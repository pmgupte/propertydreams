...
class iproperty {
	...
	private $login_page = 'http://irealtor.iproperty.com.sg/default.aspx?err=1&rd=http://irealtor.iproperty.com.sg/dashboard.aspx';
	private $logout_page = '';
	...
	public function post_ad($data) {
		...
		// echo $response; exit; 
		if(empty($response)) { return array('code'=>false, 'message'=>'Could not execute curl at Step 1'); }
		...
		// login
		// echo "sending POST request: $postfields";
		$ch = curl_init($this->login_page);
		//curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.9.2.24) Gecko/20111107 Ubuntu/10.04 (lucid) Firefox/3.6.24');
		...
		if(empty($response)) { return array('code'=>false, 'message'=>'Could not execute curl at Step 2.'); }
	
		// GET http://irealtor.iproperty.com.sg/ajax_server/svr_validate.aspx?t=3&id=NqJYWBInFBk1&key=KgqwOhWnslYr0rC0hpkJ2g2
		$ajax_url = "http://irealtor.iproperty.com.sg/ajax_server/svr_validate.aspx?t=3&id=NqJYWBInFBk1&key=KgqwOhWnslYr0rC0hpkJ2g2";
		// echo " sending GET to $ajax_url ";
		...
		// print_r(curl_getinfo($ch));
		// echo $response; exit; 
		if(empty($response)) { return array('code'=>false, 'message'=>'Could not execute curl at Step 3.1'); }

		// GET http://irealtor.iproperty.com.sg/ajax_server/authenticate/svr_authenticate.ashx?Source=2&UserName=USERNAME&PassWord=PASSWORD
		$ajax_url = "http://irealtor.iproperty.com.sg/ajax_server/authenticate/svr_authenticate.ashx?Source=2&UserName={$data['username']}&PassWord={$data['password']}";
		// echo " sending GET to $ajax_url ";
		curl_setopt($ch, CURLOPT_URL, $ajax_url);
		curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.9.2.24) Gecko/20111107 Ubuntu/10.04 (lucid) Firefox/3.6.24');
		curl_setopt($ch, CURLOPT_REFERER, $this->login_page);
		...
		$response = curl_exec($ch);
		// print_r(curl_getinfo($ch));
		// if(empty($response)) { return array('code'=>false, 'message'=>'Could not execute curl at Step 3.2'); }
		...
		//echo $response;
		if(!is_numeric($pid)) { echo $pid; return array('code'=>false, 'message'=> 'Non-numeric pid found. Cannot continue.'); }
