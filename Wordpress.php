...
$data['username'] = $_SESSION['credentials']['Wordpress']['username'];
$data['password'] = $_SESSION['credentials']['Wordpress']['password'];

if(empty($data['username']) || empty($data['password'])) {
	echo "Username/Password for Wordpress not set. Skipping...";ob_flush();
}
...
{
	$data['handle'] = $_SESSION['handle']['Wordpress'];
	echo "Inside delete";
	echo "Deleting ad from Wordpress...";ob_flush();
	include_once 'modules/wordpress/main.php';
	$object = new wordpress();
	 $ret_val = $object->delete_ad($data); // $data is associative array containing login credentials and handle 
            
                if($ret_val['code']){
			// Delete from db
		$query = "SELECT Site_ID FROM $tbl_name_sites WHERE Site_Name='Wordpress'";
		...
		$_SESSION['responses']['Wordpress'] = $ret_val;
	...
	echo "Posting ad to Wordpress...";ob_flush();
	include_once 'modules/wordpress/main.php';
	$object = new wordpress();
	...
	$_SESSION['responses']['Wordpress'] = $response;
	$_SESSION['processing_site'] += 1;
	$location = '';
