...
<div class="step-<?php echo ($step_number==2?"dark":"light");?>-left"><a href="create_Ad.php">Basic Information</a></div>
...
function create_ad_Location_Details() {?>
		...
		<tr>
			...
			<td>
				<?php $district= array(
				   ...
				   <select name="district" id="district" class="styledselect_form_1">
						<?php 
		...
		<td>
		<select name="estate" id="estate" class="styledselect_form_1">
				<?php
				$estate=array(
						"(select)",
						"Admiralty",
		...
		<tr>
			...
			<?php $Nearest_MRT=array("Admiralty_NS10",
						"Aljunied_EW9",
						"Ang_Mo_Kio_NS16",
						"Bakau_LRT",
						"Bangkit_LRT",
						...
						"Farrer_Park_NE8",
						"Fernvale_LRT",
						"HarbourFront_NE1-CC29",
						"Haw_Par_Villa_CCL",
						"Holland Village_CCL",
						"Hougang_NE14",
						"Jelapang_LRT",
						"Joo_Koon_EW29",
						"Jurong_East_NS1-EW24",
						"Kadaloor_LRT",
						"Kallang_EW10",
						"Kangkar_LRT",
						"Keat_Hong_LRT",
...
?>
			...
			<th valign="top">Nearest MRT:<br>(<font color="red">* </font>for rentinsingapore)</th>
			<td>
						   <select name="$Nearest_MRT" id="$Nearest_MRT" class="styledselect_form_1">
						<?php 
						foreach($Nearest_MRT as $d) {?>
								<option value="<?php echo $d;?>" <?php if(isset($_SESSION['$Nearest_MRT']) && ($_SESSION['$Nearest_MRT'] == $d)){ echo "selected";}?>> <?php echo $d; ?></option>
	...
	$Nearest_LRT=array(			"(select)",
			...
			<th valign="top">Nearest LRT:<br>(<font color="red">* </font>for rentinsingapore)</th>
			<td>
						   <select name="$Nearest_LRT" id="$Nearest_LRT" class="styledselect_form_1">
						<?php 
						foreach($Nearest_LRT as $d) {?>
								<option value="<?php echo $d;?>" <?php if(isset($_SESSION['$Nearest_LRT']) && ($_SESSION['$Nearest_LRT'] == $d)){ echo "selected";}?>> <?php echo $d; ?></option>
...
window.open(theURL,winName,features).focus();
				...
				echo "You don't have any ad posted. Would you like to <a href='create_Ad.php'>post a new one</a>?";		
				}
}
...
$query= "SELECT Property_Name,Location,Ads_ID,Type,Price,date_Posted,District FROM $tbl_name_Ad WHERE status ='1'";
				...
				while($row = mysql_fetch_array($result))
				{
					...
					<a href="repost.php?Ads_ID=<?php echo $Ads_ID?>" title="Repost" class="icon-2 info-tooltip"></a>
